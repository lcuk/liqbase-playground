...
liqcell *ciroclock_minutes_12h_time_create()
{
	liqcell *self = liqcell_quickcreatewidget("ciroclock_minutes_12h_time","form", 400,240);
	...
	if(self)
	{
		...
		liqcell_child_insert( self, liqcell_quickcreatevis("clockhum",   "time",   0,200,   400,40 )    );

		liqcell_propsets(     liqcell_child_lookup( self,"clockhum"),"timeformat","human");
		liqcell_propseti(     liqcell_child_lookup( self,"clockhum"),"textalign", 2);
		liqcell_propseti(     liqcell_child_lookup( self,"clockhum"),"textaligny",2);
		liqcell_propsets(     liqcell_child_lookup( self,"clockhum"),"fontname", "/usr/share/fonts/nokia/nosnb.ttf" );
		liqcell_propseti(     liqcell_child_lookup( self,"clockhum"),"fontsize", 20 );



		//liqcell_handleradd_withcontext(self,"paint",clock_paint,self);
