...
//#####################################################################
//#####################################################################
//##################################################################### liqrecentsketchesrot :: by gary birkett 
//#####################################################################
//#####################################################################
/*

	static int liqrecentsketchesrot_sketchitem_click(liqcell *self, liqcellclickeventargs *args, liqcell *liqrecentsketchesrot)
	{
 ...
 */
	...
	int liqrecentsketchesrot_sketchitem_click(liqcell *self, liqcellclickeventargs *args, void *context)
	{
...
static int liqrecentsketchesrot_sketch_insert(liqcell *self,char *filenamebuffer)
{
	...
	if(	(pagefilename_breakapart(&pfn,filenamebuffer) == 0) )
	{
		...
		//liqapp_log("ins 3 %s",filenamebuffer);
			...
			//liqapp_log("ins 5 %s",filenamebuffer);
				...
				liqcell_handleradd_withcontext(c,"click",liqrecentsketchesrot_sketchitem_click,self);
...
static int liqrecentsketchesrot_sketch_add(liqcell *self,char *filenamebuffer)
{
	...
	if(	(pagefilename_breakapart(&pfn,filenamebuffer) == 0) )
	{
		liqrecentsketchesrot_sketch_insert(self,filenamebuffer);
		return 0;
/*		
		char buf[FILENAME_MAX];			snprintf(buf,sizeof(buf),"%s%s",pfn.filedate,pfn.filetitle);
		...
		while( NULL != (dir_entry_p = readdir(dir_p)))
		{
			...
			if ( S_ISREG(statbuf.st_mode) )
			{
				...
				if(	stristr(ft,"liq.") && (pagefilename_breakapart(&pfn,ft) == 0) )
				{
					...
					liqrecentsketchesrot_sketch_insert(self,fn);
...
liqcell *liqrecentsketchesrot_create()
{
	liqcell *self = liqcell_quickcreatewidget("liqrecentsketchesrot","form", 800,480);
	  ...
	  if ( (pevent->len) )
	  {
		...
		// dealing with a file
		  if(pevent->mask & IN_CLOSE_WRITE)
			liqrecentsketchesrot_sketch_add(context,action);
	  }
