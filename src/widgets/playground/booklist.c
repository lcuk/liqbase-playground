		...
		while( NULL != (dir_entry_p = readdir(dir_p)))
		{
			...
			if ( S_ISREG(statbuf.st_mode) )
			{
				  ...
				  )
				{
					...
					liqfont *font = liqfont_cache_getttf("/usr/share/fonts/nokia/nosnb.ttf", (18 ), 0);
...
//#####################################################################
	...
	static int booklist_filter(liqcell *self, liqcellfiltereventargs *args, void *context)
	{
	...
	static int booklist_refresh(liqcell *self, liqcelleventargs *args, void *context)
	{
	...
	static int booklist_resize(liqcell *self, liqcelleventargs *args, void *context)
	{
...
liqcell *booklist_create()
{
	liqcell *self = liqcell_quickcreatewidget("booklist","form", 800,480);
	...
	if(self)
	{
		...
		liqcell_handleradd(self,    "refresh",   booklist_refresh);
		liqcell_handleradd(self,    "resize",   booklist_resize);
		liqcell_handleradd(self,    "filter",   booklist_filter);

		booklist_resize(self,NULL,NULL);
