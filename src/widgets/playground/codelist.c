		...
		while( NULL != (dir_entry_p = readdir(dir_p)))
		{
			...
			if ( S_ISREG(statbuf.st_mode) )
			{
				  ...
				  )
				{
					...
					liqcell *c = liqcell_quickcreatevis(ft,   "code",   0,0,400,40    );
					//liqcell_propseti(c,"lockaspect",1);
					liqcell_propsets(c,"imagefilename","media/quickicons/gnome-terminal.png");
					//liqcell_handleradd(c,    "mouse",   widget_mouse);
					liqcell_handleradd(c,    "click",   item_click);
					

						liqcell_setfont(	c, liqfont_cache_getttf("/usr/share/fonts/nokia/nosnb.ttf", (18), 0) );
						liqcell_propsets(  c, "textcolor", "rgb(255,255,255)" );
						liqcell_propsets(  c, "backcolor", "rgb(0,60,0)" );
				  ...
				  )
				{
					...
					liqcell_handleradd(c,    "click",   item_click);
						...
						liqcell_setfont(	c, liqfont_cache_getttf("/usr/share/fonts/nokia/nosnb.ttf", (18), 0) );
						liqcell_propsets(  c, "textcolor", "rgb(255,150,255)" );
						liqcell_propsets(  c, "backcolor", "rgb(60,60,0)" );
						liqcell_propseti(  c, "textalign", 2 );
						liqcell_propseti(  c, "textaligny",2 );
...
//#####################################################################
	...
	static int codelist_filter(liqcell *self, liqcellfiltereventargs *args, void *context)
	{
		// using the filter provided (which might be blank)
		
					int islike = liqcell_propgeti(  self, "filterlike", 1 );
	...
	static int codelist_refresh(liqcell *self, liqcelleventargs *args, void *context)
	{
	...
	static int codelist_resize(liqcell *self, liqcelleventargs *args, void *context)
	{
	...
	static int codelist_paint(liqcell *self, liqcellpainteventargs *args,liqcell *codelist)
	{
		liqcell *body = liqcell_child_lookup(self,"body");
...
liqcell *codelist_create()
{
	liqcell *self = liqcell_quickcreatewidget("codelist","form", 800,480);
	...
	if(self)
	{
		...
		liqcell_handleradd(self,    "refresh",   codelist_refresh);
		liqcell_handleradd(self,    "resize",   codelist_resize);
		liqcell_handleradd(self,    "filter",   codelist_filter);
		liqcell_handleradd(self,    "paint",   codelist_paint);
		codelist_resize(self,NULL,NULL);
