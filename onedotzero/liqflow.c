...
static int accel_read(int *ax,int *ay,int *az)
{
	...
	//osc_onedotzero_send_acc(*ax,*ay,*az);
    
    
	osc_onedotzero_send_shake(*ax* 180 / 1000,*ay* 180 / 1000,*az* 180 / 1000,accel_fat);
...
static void star_calc(liqcell *liqstars,STAR *stars,vgraph *graph,liqsketch *sketch, int drawwidth,int drawheight)
{
    
    
    liqcell *starx = liqcell_child_lookup(liqstars, "starx");
    liqsketch *starsketch = (starx ? liqcell_getsketch(starx) : NULL);
    liqsketch *starimg = (starx ? liqcell_getimage(starx) : NULL);
...
moo:
	...
	*/	
		...
		if(starsketch)
        {
            ...
            while(s)
            {
                s->pen_y = 255;//lim( (r*255)-128 );
                s->pen_u = lim((int)sa->v.x);
                s->pen_v = lim((int)sa->v.y);
                
                s=s->linknext;
            }
            //vgraph_drawsketch(graph, sa->scrx-25,sa->scry-25,25,25,starsketch);
            vgraph_drawsketch(graph, sa->scrx-hsize,sa->scry-hsize,size,size,starsketch);
        }
        else
        {
            ...
            if(starimg)
            {
                ...
                vgraph_drawimage(  graph, sa->scrx-hsize,sa->scry-hsize,size,size,starimg);
                
                
                
                vgraph_setbackcolor(graph, vcolor_YUV(magmag,  lim((int)sa->v.x),lim((int)sa->v.y)    ) );
                vgraph_drawrectwash(  graph, sa->scrx-hsize,sa->scry-hsize,size,size);
            ...
            else
            {
                ...
                int hsize = size >> 1;
                vgraph_setbackcolor(graph, vcolor_YUV(magmag,  lim((int)sa->v.x),lim((int)sa->v.y)    ) );
                vgraph_drawrect(  graph, sa->scrx-hsize,sa->scry-hsize,size,size);
            }
        }
	...
	static int sketchedit_mouse(liqcell *self, liqcellmouseeventargs *args, void *liqflow)
	{
		...
		else
		{
			...
			if(p)
			{
				...
				//onedotzero_scs_sendserv(liqflow,msg);
			}
 ...
 *  cmdclear clicked
 */	
static int cmdclear_click(liqcell *self,liqcelleventargs *args, liqcell *liqflow)
{
		liqsketch *sketch = liqcell_getsketch(liqflow);
		if(sketch)
		{
			liqsketch_clear(sketch);
		}
		star_init_all( (STAR *)liqflow->tag  );
		
		
		//onedotzero_scs_sendserv(liqflow,"clear\n");
...
/*
	...
	static int timer_tick(liqcell *self, liqcellmouseeventargs *args, void *liqflow)
	{
		...
		while(1)
		{
			...
			onedotzero_scs_session(liqflow, liqapp_pref_getvalue_def("scsserver","") ,12345);
...
static int widget_dialog_close(liqcell *self,liqcelleventargs *args, liqcell *context)
{
        ...
        osc_onedotzero_send_menufinished(1);
...
liqcell *liqflow_create()
{
	liqcell *self = liqcell_quickcreatewidget("liqflow","form", 800,480);
	...
	//liqcell_propsets(  cmdclear, "bordercolor", "rgb(200,100,100)" );
	liqcell_setimage(  cmdclear,  liqimage_cache_getfile("/usr/share/liqbase/onedotzero/media/button_back.png", 0,0,1) );
	liqcell_propseti(  cmdclear, "textalign", 2 );
    ...
    */	
	...
	liqcell_setcaption(aname, "liqflow" );
	...
	//liqcell_propsets(  ainvite, "bordercolor", "rgb(200,100,100)" );
	liqcell_setimage(  ainvite,  liqimage_cache_getfile("/usr/share/liqbase/onedotzero/media/button_back.png", 0,0,1) );
	liqcell_propseti(  ainvite, "textalign", 2 );
	liqcell_propseti(  ainvite, "textaligny", 2 );
	...
	liqcell_child_append(  self, ainvite);
        ...
        //liqcell_setsketch(starx,liqsketch_newfromfile("/usr/share/liqbase/onedotzero/media/star.sketch"));
        liqcell_setimage( starx,liqimage_cache_getfile( "/usr/share/liqbase/onedotzero/media/blob.png",0,0,1));
