...
//#####################################################################
	...
	static int sketchedit_mouse(liqcell *self, liqcellmouseeventargs *args, void *liqflow_tiny)
	{
 ...
 *  cmdclear clicked
 */	
static int cmdclear_click(liqcell *self,liqcelleventargs *args, liqcell *liqflow_tiny)
{
		liqsketch *sketch = liqcell_getsketch(liqflow_tiny);
		if(sketch)
		{
			liqsketch_clear(sketch);
		}
		star_init_all( (STAR *)liqflow_tiny->tag  );
...
liqcell *liqflow_tiny_create()
{
	liqcell *self = liqcell_quickcreatewidget("liqflow_tiny","form", 800,480);
	...
	//	liqcell_propsets(  self, "bordercolor", "rgb(0,150,0)" );
     ...
     //   liqsketch_fileload(sketch, "/usr/share/liqbase/media/flowx.sketch" );
...
/*			
	...
	liqcell_setcaption(aname, "liqflow_tiny" );
