...
// this is only used when liqmic is compiled as a standalone binary
...
static void liqcell_easyrun_internal(liqcell *dialog)
{
	 ...
	 {
		  liqapp_errorandfail(-1,"liqmic canvas Init");
		  //closeall();
		  return -1;
	 }
...
{
	 if(liqapp_init(	argc,argv ,"liqmic" ,"0.0.1") != 0)
	 {
		  { return liqapp_errorandfail(-1,"liqmic liqapp_init failed"); }
	 }
	 liqcell *self = liqcell_quickcreatevis("liqmic_cover1", "liqmic_cover", 0,0, -1,-1);
	 // remove tools button :)
