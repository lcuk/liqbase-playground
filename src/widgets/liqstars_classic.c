...
static void eep_star_init_one(STAR *sa)
{
...
static void eep_star_init_all(STAR *stars)
{
...
static void eep_star_calc(STAR *stars,vgraph *graph)
{
...
#define RNG 880
	...
	//liqapp_log("stars_classic init one");
...
void star_init_all(STAR *stars)
{
	//liqapp_log("stars_classic init all");
	int a;
	STAR *sa;
	for(a=0;a<starcount;a++)
	{
...
void star_calc(STAR *stars,vgraph *graph)
{
	//liqapp_log("stars_classic calc");
	int a;
	STAR *sa;
	// weigh first, this does not effect the location
...
moo:
		...
		// classic //canvas_pset(sa->scrx,sa->scry,sa->scrcol);
		// classic //canvas_pset(sa->scrx+1,sa->scry,sa->scrcol);
...
liqcell *liqstars_classic_create()
{
	liqcell *self = liqcell_quickcreatewidget("liqstars_classic","form", canvas.pixelwidth,canvas.pixelheight);
