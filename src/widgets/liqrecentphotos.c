...
static int liqrecentphotos_item_add(liqcell *self,char *filenamebuffer)
{
	...
	if(	(pagefilename_breakapart(&pfn,filenamebuffer) == 0) )
	{
		...
		liqcell *body = liqcell_child_lookup( self,"body");
						...
						liqcell *c = liqcell_quickcreatevis(buf,   "picture",   1,1,1,1    );
...
liqcell *liqrecentphotos_create()
{
	...
	//	liqcell_propsets(  self, "backcolor", "rgb(255,255,0)" );
		...
		char buf[FILENAME_MAX];    snprintf(buf,sizeof(buf),"%s/MyDocs/.camera",app.homepath);
...
if(liqapp_pathexists(buf))
{
	...
	liqapp_log("recent sketches buf=%s",buf);
		...
		//b = liqcell_quickcreatevis("TREE","button",  800-160,480-160,   160,160 );
		...
		//liqcell_handleradd_withcontext(   b,   "click",   viewtree_click, self);
...
static void monitor_get_event(int fd, const char * target,liqcell *context)
{
   ...
   while (i < len)
   {
	  ...
	  if ( (pevent->len) )
	  {
		...
		//    liqrecentphotos_sketch_add(context,pevent->name);
	  }
/*
      if (pevent->len) 
