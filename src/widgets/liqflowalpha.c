...
//#####################################################################
	...
	static int sketchedit_mouse(liqcell *self, liqcellmouseeventargs *args, void *liqflowalpha)
	{
		...
		else
		{
			...
			if(p)
			{
				...
				//onedotzero_scs_sendserv(liqflowalpha,msg);
			}
 ...
 *  cmdclear clicked
 */	
static int cmdclear_click(liqcell *self,liqcelleventargs *args, liqcell *liqflowalpha)
{
		liqsketch *sketch = liqcell_getsketch(liqflowalpha);
		if(sketch)
		{
			liqsketch_clear(sketch);
		}
		star_init_all( (STAR *)liqflowalpha->tag  );
...
/*
	...
	static int timer_tick(liqcell *self, liqcellmouseeventargs *args, void *liqflowalpha)
	{
		...
		while(1)
		{
			...
			onedotzero_scs_session(liqflowalpha, liqapp_pref_getvalue_def("scsserver","") ,12345);
...
liqcell *liqflowalpha_create()
{
	liqcell *self = liqcell_quickcreatewidget("liqflowalpha","form", 800,480);
	...
	//liqcell_propsets(  cmdclear, "bordercolor", "rgb(200,100,100)" );
	liqcell_setimage(  cmdclear,  liqimage_cache_getfile("/usr/share/liqbase/media/button_back_dark.png", 0,0,1) );
	liqcell_propseti(  cmdclear, "textalign", 2 );
    ...
    */	
	...
	liqcell_setcaption(aname, "liqflowalpha" );
